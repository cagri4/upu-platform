...
export async function handleAnaliz(ctx: WaContext): Promise<void> {
  try {
    ...
    await logEvent(ctx.tenantId, ctx.userId, "analiz", `${totalProps || 0} aktif ilan`);
  } catch (err) {
    await handleError(ctx, "emlak:analiz", err, "db");
  }
}
...
export async function handleRapor(ctx: WaContext): Promise<void> {
  try {
    ...
    await logEvent(ctx.tenantId, ctx.userId, "rapor", `${propRes.count || 0} mülk, ${custRes.count || 0} müşteri`);
  } catch (err) {
