  ...
  try {
    ...
    if (!campaigns || campaigns.length === 0) {
      await sendButtons(ctx.phone,
        "📭 Şu anda aktif hediye/kampanya yok.\n\nYeni kampanyalar eklendiğinde sekreteriniz size bildirecek.",
    ...
    await sendList(ctx.phone, text + "Detay için seçin:", "Kampanya Seç", [
      { title: "Kampanyalar", rows },
    ]);
    await logEvent(ctx.tenantId, ctx.userId, "hediyeler", `${campaigns.length} kampanya listelendi`);
  } catch (err) {
    await handleError(ctx, "emlak:hediyeler", err, "db");
  }
}
...
export async function handleHediyelerCallback(ctx: WaContext, data: string): Promise<void> {
  try {
    ...
    await handleError(ctx, "emlak:hediyeler_cb", err, "db");
  }
}
