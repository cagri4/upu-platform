  ...
  const cta = ctaLabel
    ? ctaHref
      ? (
          <a
            href={ctaHref}
            className="inline-flex items-center mt-4 bg-white text-amber-700 hover:bg-amber-50 px-4 py-2 rounded-lg text-sm font-semibold transition active:scale-95"
          >
            {ctaLabel}
          </a>
        )
    ...
    <div className="bg-gradient-to-br from-amber-500 to-orange-600 dark:from-amber-600 dark:to-orange-700 text-white rounded-2xl p-5 sm:p-6 shadow-md">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {subtitle && (
            <p className="text-amber-50/95 text-sm mt-1.5 leading-relaxed">{subtitle}</p>
          )}
          {cta}
        </div>
      </div>
