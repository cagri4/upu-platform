...
function List({ token }: { token: string }) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-1">💝 Müdavimler</h1>
      {items && (
      ...
      {birthdaysToday.length > 0 && (
        <div className="bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800/50 rounded-xl p-4 mb-5">
      ...
      {dormant.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl p-4 mb-5">
            ...
            return (
              <div key={m.id} className="p-4 sm:p-5">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    <div className="font-medium text-slate-900 dark:text-slate-100">{m.guest_name}</div>
                    {m.guest_phone && <div className="text-xs text-slate-500 mt-0.5">{m.guest_phone}</div>}
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{m.visit_count}× · {fmtEur(m.total_spent)}</div>
