...
export default function UrunDetayPage() {
  ...
  const stockBadgeCls = {
    out: "bg-rose-50 text-rose-700 font-semibold",
    critical: "bg-amber-50 text-amber-700 font-semibold",
        ...
        <section className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className="p-4">
            ...
            <div className="flex flex-wrap gap-2 mt-2 text-xs">
              {product.category && (
                <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{product.category}</span>
              )}
              {product.code && (
                <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">{product.code}</span>
              )}
              ...
              {recentOrders.map(o => (
                <div key={o.orderId} className="flex items-center justify-between border border-slate-100 rounded-lg p-2.5 hover:bg-slate-50">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-slate-800 truncate">{o.dealerName}</div>
