...
/**
 * /tr/davet/[code] — Davet kabul sayfası.
 *
 * Flow:
 *   1. GET /api/bayi-davet/validate?code= → davet detay
 *   2. "Devam Et" → POST /api/bayi-davet/accept { code }
 *   3. Cookie session attach + redirect /tr/bayi-panel
 */
...
export default function DavetPage() {
  ...
  const [state, setState] = useState<"loading" | "ready" | "error" | "accepting" | "done">("loading");
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-md p-6 space-y-4">
        ...
        {(state === "ready" || state === "accepting") && data && (
          <>
            ...
            >
              ...
              {state === "accepting" ? "Hesap oluşturuluyor…" : "Devam Et"}
              {state !== "accepting" && <ArrowRight className="w-4 h-4" />}
            </button>
          </>
        )}
