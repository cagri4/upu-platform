...
const STATUS_LABEL: Record<string, { label: string; color: string }> = {
  ...
  checked_out:  { label: "Çıktı",        color: "bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300" },
      ...
      {list && list.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
          <ul className="divide-y divide-slate-100">
            {list.map((r) => {
              const status = STATUS_LABEL[r.status || ""] || { label: r.status || "—", color: "bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300" };
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {r.pre_checkin_complete && (
                          <span className="text-[11px] px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700" title="Online çek-in tamam">📝✓</span>
                        )}
                      </div>
                    ...
                    {r.total_price !== null && (
                      <div className="text-right shrink-0">
                        <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{Number(r.total_price).toLocaleString("tr-TR")} ₺</div>
                      </div>
                    )}
                  </div>
                </li>
