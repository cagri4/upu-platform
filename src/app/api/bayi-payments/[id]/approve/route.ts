/**
 * POST /api/bayi-payments/[id]/approve — admin/muhasebe ödemeyi onaylar.
 * pending → approved + approved_by_user_id + approved_at. Bayi'ye WA bildirim.
 */
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (payment.status !== "pending") {
    return NextResponse.json({ error: `Sadece bekleyen ödeme onaylanabilir (mevcut: ${payment.status}).` }, { status: 409 });
  }
  ...
  void notifyDealerPaymentDecision(sb, payment.dealer_user_id, Number(payment.amount), true);
