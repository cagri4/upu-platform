/**
 * /api/otel-panel/evergreen?uid=<user_id>  (preferred — multi-tenant safe)
 * /api/otel-panel/evergreen?phone=<phone>  (legacy fallback)
 *
 * "Süresi dolmaz" otel panel link'i. WA mesajlarındaki "🖥 Paneli Aç" CTA'ları
 * bu URL'e iner; tıklandığında server fresh magic_link_token mint edip
 * /tr/otel-panel'e 302'ler.
 *
 * Pattern: bayi-panel/evergreen ile aynı; otel tenant guard +
 * hotelai.upudev.nl default.
 */
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.redirect(`${APP_URL}/tr/otel-panel?t=${token}`);
}
