#!/usr/bin/env node
/**
 * Sahibinden Scraper V2 — Liste sayfasından 10 bilgi, detay sayfasına girilmez
 *
 * Usage:
 *   node scripts/scrape-v2.mjs          → Full scrape (42 URL)
 *   node scripts/scrape-v2.mjs --daily  → Sadece yeni ilanlar (?date=1day)
 *   node scripts/scrape-v2.mjs --visible → Tarayıcıyı göster
 *   node scripts/scrape-v2.mjs --test   → İlk 3 URL ile test
 */
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'v2-bodrum.json');
...
function loadCookies() {
  ...
  if (!fs.existsSync(cookiePath)) {
    console.error('❌ cookies.json bulunamadı! scripts/cookies.json oluştur.');
    process.exit(1);
  }
...
// Her satırdan 10 bilgi çıkar: title, m², rooms, price, date, mahalle, url, photo, listing_type, property_type
// listing_type ve property_type URL'den gelir, geri kalan 8'i DOM'dan.

function parseRow(tds, propertyType) {
  // tds = [{text, class}] dizisi — page.evaluate'den geliyor
  const n = tds.length;

  // Sondan sayma (tüm tipler için geçerli):
  // Son td = ignore (fav/checkbox)
  // Sondan 2. = Mahalle
  // Sondan 3. = Tarih
  // Sondan 4. = Fiyat
  // mahalle artık extractListings'ten locText olarak geliyor, parseRow'a ayrı parametre
  const mahalle = ''; // locText ile override edilecek
  ...
  if (ARSA_TYPES.has(propertyType)) {
    // Arsa: [0]=foto, [1]=başlık, [2]=m², [3]=fiyat, [4]=m² birim fiyat, ...sondan
    m2 = parseArea(tds[2]?.text || '');
    // Arsa'da oda yok
  } else if (DEVRE_MULK_TYPES.has(propertyType)) {
    // Devre Mülk: [0]=foto, [1]=tesis adı, [2]=başlık, [3]=hafta, [4]=oda, ...sondan
    rooms = parseRooms(tds[4]?.text || '');
  } else if (BINA_TYPES.has(propertyType)) {
    // Bina: 10 sütun — [2]=m², [3]=oda/birim, ...sondan
    m2 = parseArea(tds[2]?.text || '');
    rooms = parseRooms(tds[3]?.text || '');
  } else if (OTEL_TYPES.has(propertyType)) {
    // Otel: [2]=yıldız/oda, [3]=yatak kapasitesi
    rooms = tds[2]?.text?.trim() || '';
  } else {
    // Standart konut + işyeri: 8 sütun
    // [0]=foto, [1]=başlık, [2]=m² (brüt/net), [3]=oda, ...sondan 4=fiyat, 3=tarih, 2=mahalle, 1=ignore
    m2 = parseArea(tds[2]?.text || '');
    rooms = parseRooms(tds[3]?.text || '');
  }
...
function extractListings() {
  ...
  for (const row of rows) {
    ...
    // Location td — <br> ile ayrılmış semt/mahalle
...
async function scrape() {
  ...
  // Resume (full scrape) veya sıfırdan başla (daily/days mode)
  const progress = isDateFiltered
    ? { completedUrls: [], listings: [] }
    : (loadJSON(PROGRESS_FILE) || { completedUrls: [], listings: [] });
  ...
  if (allListings.length > 0) {
    console.log(`📂 Resume: ${allListings.length} ilan${isDailyMode ? '' : `, ${completedUrls.size} URL tamamlanmış`}\n`);
  }
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    while (true) {
      ...
      if (pageUrl.includes('/giris') || pageUrl.includes('/login')) {
        console.error('  ❌ Giriş duvarı! Cookie güncelle.');
        blocked = true;
        break;
      }
      ...
      if (rawListings.length === 0) {
        console.log('  Sayfada ilan yok — bitiş.');
        break;
      }
  ...
  for (const l of allListings) {
    const key = `${l.listing_type}/${l.property_type}`;
  ...
  console.log(`\n📁 Çıktı: ${OUTPUT_FILE}`);
  if (blocked) console.log(`\n⚠️ Engel nedeniyle durdu. Tekrar çalıştırınca kaldığı yerden devam eder.`);
}
